...
\begin{exercicio}
  ...
  árvores cuja altura é um limite. O item (d) do Exemplo 3.2 nos diz que
...
\begin{proof}
 ...
 \[\{a\in T\,\colon  a\leq c_n \text{ para algum }
  ...
  \end{proof}
    ...
    Observe que no resultado acima usamos o Teorema da Recursão sem
    explicitamente especificar uma função \(g\) para
    calcular \(c_{n+1}\) a partir de \(c_n\). De fato, para consturir tal função precisamos
    aplicar o Axioma da Escolha. De fato, se \(k\) é uma função escolha para
    ...
    conforme o Teorema da Recursão que vimos. O próximo exercício será uma
    generalização do Lema de König.
  
  \begin{exercicio}
  Seja \((T,\leq)\) uma árvore de altura $\kappa$ ($\kappa$ um cardinal
  infinito), com todos os níveis finitos. Prove que T possui um ramo de tamanho
  $\kappa$.
  \end{exercicio}
  \begin{proof}[Solução]
