...
\begin{exercicio}
  ...
  \begin{exemplo}\hfill
   ...
   \item Seja \(A=\Naturals \). Consideremos
    \(T\subseteq \Naturals^{<\omega}\)
    o conjunto das sequências de números naturais que são finitas e
  ...
  \item Seja \((R,\leq)\) um conjunto linearmente ordenado.
    ...
    a árvore \(S=Seq(\{0,1\})=\{0,1\}^{<\omega}\) ordenado por \(\subseteq\),
  ...
  árvores cuja altura é um limite. O Exemplo 3.2(d) nos diz que
  existem árvores de altura \(\omega\) que possuem apenas ramos finitos. O
  próximo teorema, que é a observação mais básica com alguma relevância
  para nossos estudos, mostra que isso não pode acontecer
  se a árvore é suficientemente "esguia".

  \begin{teo}[Lema de König]
  Se \(T\) é uma árvore de altura \(\omega\), com todos os níveis finitos, então
  \(T\) admite um ramo de altura \(\omega\).
  \end{teo}
  \begin{proof}
    Equivalentemente, temos que provar que, toda árvore de altura \(\omega\),
    tal que cada nó tem um número finito de sucessores imediatos, tem um ramo infinito.
  De fato vamos construir, com recursão, uma sequência(infinita) \(\langle c_n\rangle_{n=0}^{\infty}\) de nós de \(T\) tal que, para todo \(n\), \(\{a\in T\,\colon c_n\leq a\}\) é infinito. Seja \(c_0\) a raiz de T, note que \(\{a \in T\,\colon \mid c_{0} \leq a \}= T\) é finito. Como
  \[
  \{a\in T\,\colon c_n\leq a\} = \{c_n\} \cup\bigcup_{b\in S} \{a\in T\,\colon b\leq a\}
  \]
  onde \(S\) é o conjunto dos sucessores imediatos de \(c_n\). Logo, para pelo menos um \(b\in S\), \(\{a\in T\,\colon b\leq a\}\) é infinito, assim defina \(c_{n+1}\) um tal \(b\). 
  Falta verificar que \(\{a\in T\,\colon c_n\geq a \text{ para algum } n\}\) é um ramo de \(T\) com altura \(\omega\).
  ...
  \begin{exercicio}
  Seja \((T,\leq)\) uma árvore de altura $\kappa$ ($\kappa$ um cardinal infinito), com todos os níveis finitos. Prove que T possui um ramo de tamanho $\kappa$.
  \begin{proof}[Solução]
